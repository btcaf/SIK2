...
Temp_Sender::Temp_Sender(uint64_t _data_port, size_t _packet_size, std::string _name,
                std::string _multicast_address_string, uint16_t _ctrl_port, size_t _queue_size,
...
Sender Temp_Sender::make_sender() {
    struct sockaddr_in multicast_address = get_address(multicast_address_string, data_port, true);
    ...
    return {packet_size, multicast_address, data_socket_fd, ctrl_socket_fd, queue_size, std::chrono::milliseconds(rexmit_time), reply_message};
}